...
- 不要阅读或依赖脚本内部实现，只把自动化脚本当作 API 使用。
- 脚本调用同一动作连续失败 `3` 次后，停止自动重试并切换为手动模式。
...
6. `contracts/actions`
   对 AI 暴露稳定的动作接口，只描述输入、输出、错误和重试规则。
7. `artifacts/manifests/state`
   保存产物、元数据和生命周期状态。
...
- `docs/api-contract.md`：给 AI 读取的动作 API 契约。
...
自动化脚本的定位不是“给 AI 阅读的源码”，而是“给 AI 调用的受控 API”。

设计要求：

- AI 只读取动作契约，不读取脚本实现。
- 每个动作必须有稳定名称、输入参数、输出字段、错误码和重试规则。
- 脚本输出必须结构化，优先使用 JSON。
- 脚本参数要足够完整，避免 AI 为补参数而阅读实现细节。
- 同一动作连续失败 `3` 次后，必须进入手动模式，不再自动重试。
...
动作契约见 [api-contract.md](/Users/regen-bio/CODE/Big_data_factory/docs/api-contract.md)。
...
```bash
python -m big_data_factory.cli describe-actions
```
